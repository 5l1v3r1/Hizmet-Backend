...
@section('title')
    {{ trans('devices.relay_detail_title') }}
@endsection
...
@section('content')
    <?php
        $the_relay = json_decode($the_device);
    ?>

    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row" id="div_relay_summary" style="margin-bottom:20px;">
            <div class="col-md-6">
                <div class="profile-image">
                    <img src="/img/avatar/client/{{$the_relay->client_avatar}}" class="img-circle circle-border m-b-md" alt="profile">
                </div>
                <div class="profile-info">
                    <div>
                        ...
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.trademark_model') }}:  <strong> {{ $the_relay->trademark." / ". $the_relay->model." (".trans("global.".$the_relay->connection_type).")" }} </strong>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.modem_no') }}:  <a href="/modem_management/detail/{{$the_relay->modem_id}}" target="_blank"><strong> {{ $the_relay->modem_serial }} </strong></a>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.client_distributor') }}:
                            <a title="{{ trans('devices.go_client_detail') }}" href="/client_management/detail/{{$the_relay->client_id}}" target="_blank">
                                <strong> {{ $the_relay->client_name . " / " }} </strong>
                            </a>
                            @if ( $the_relay->distributor_id != 0 )
                                <a title="{{ trans('devices.go_distributor_detail') }}" href="/distributor_management/detail/{{$the_relay->distributor_id}}" target="_blank">
                                    <strong> {{ $the_relay->distributor }} </strong>
                                </a>
                            @else
                                <strong> {{ $the_relay->distributor }} </strong>
                            @endif
                        ...
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.address') }}:  <strong> {{ $the_relay->location_text }} </strong>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.explanation') }}:  <strong>
                                @if( $the_relay->explanation != "" && $the_relay->explanation != null )
                                    {{ $the_relay->explanation }}
                                @else
            ...
            <div class="col-md-6">
                <table class="table small m-b-xs">
                    ...
                    <tr>
                        ...
                        <td>
                            @if ($the_relay->last_data_at == NULL)
                                {{ trans('devices.no_first_connection') }}
                            @else
                                {{ date('d/m/Y H:i:s',strtotime($the_relay->last_data_at)) }}
                            @endif
                        </td>
                    </tr>
                    <tr>
                        ...
                        <td>
                            @if ($the_relay->last_connection_at == NULL)
                                {{ trans('devices.no_first_connection') }}
                            @else
                                {{ date('d/m/Y H:i:s',strtotime($the_relay->last_connection_at)) }}
                            @endif
                        </td>
                    </tr>
                    <tr>
                    ...
                    <tr>
                        ...
                        <td>
                            {{ date('d/m/Y H:i:s',strtotime($the_relay->created_at)) }}
                        </td>
                    </tr>
                    <tr>
                        <td>
                        ...
                        <td>
                            @if ($the_relay->expired_date == NULL)
                                {{ trans('devices.no_first_connection') }}
                            @else
                                {{ date('d/m/Y H:i:s',strtotime($the_relay->expired_date)) }}
                            @endif
                        </td>
                    </tr>
                    </tbody>
        ...
        </div> <!-- #div_relay_summary -->

        <div class="row" id="div_relay_tabs">
            <div class="col-lg-12">
                <div class="tabs-container">
                    <ul class="nav nav-tabs" id="relay_detail_tabs">
                        <li class="" tab="#tab-1">
                        ...
                        <div id="tab-1" class="tab-pane">
                                <div class="panel-body tooltip-demo" data-html="true">
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.first_data_at") }}: @if ($the_relay->first_data_at == NULL)
                                                        -
                                                    @else
                                                        {{ date('d/m/Y H:i:s',strtotime($the_relay->first_data_at)) }}
                                                    @endif</button>
                                            </div>
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.last_data_at") }}: @if ($the_relay->last_data_at == NULL)
                                                        -
                                                    @else
                                                        {{ date('d/m/Y H:i:s',strtotime($the_relay->last_data_at)) }}
                                                    @endif</button>
                                            ...
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.multiplier") }}: N/A</button>
                                            </div>
                                            <div class="col-lg-3 col-md-6">
                                    ...
                                    <div class="row">
                                        <div class="col-lg-12">
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.d_period") }}: {{ $the_relay->data_period }} dk </button>
                                            </div>
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.invoice_day") }}: {{ $the_relay->invoice_day }}</button>
                                            </div>
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{  $the_relay->fee_scale_name }}</button>
                                            </div>
                                            <div class="col-lg-3 col-md-6">
                                                <button type="button" class="btn btn-danger btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{  trans("devices.".$the_relay->fee_scale_type) }} </button>
                                            </div>
                                        </div>
                                    </div> <!-- .row -->
                                    <br/>
                        ...
                        <div id="tab-2" class="tab-pane">
                            <div class="panel-body tooltip-demo" data-html="true">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.multiplier") }}: N/A </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.current_transformer_ratio") }}: N/A
                                            </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.voltage_transformer_ratio") }}: N/A
                                            </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-danger btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.d_period") }}: {{ $the_relay->data_period }} dk</button>
                                        </div>
                                    </div>
                                </div> <!-- .row -->
...
@section('page_document_ready')
    ...
    $('#mdgraph_draw_button').click(function(){
        ...
        $.ajax({
            method:"POST",
            url:"/device_management/get_graph_data/{{ $the_relay->device_id }}",
    ...
    rememberTabSelection('#relay_detail_tabs', !localStorage);

    if (document.location.hash && document.location.hash == '#alarms') {
        $("#relay_detail_tabs a[href='#tab-8']").trigger('click');
    }
    else if(document.location.hash){
        $("#relay_detail_tabs a[href='"+document.location.hash+"']").trigger('click');
    }
    ...
    $('#relay_detail_tabs a').on('shown.bs.tab', function(event){
    ...
    active_tab = $('#relay_detail_tabs li.active').attr("tab");
    if( !(active_tab == "" || active_tab == null) )
        load_tab_content(active_tab);
    else
        $("#relay_detail_tabs a:first").trigger('click');
@endsection