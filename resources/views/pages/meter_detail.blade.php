...
@section('title')
    {{ trans('devices.meter_detail_title') }}
@endsection

@section('page_level_css')
    {!! $IndexDataTableObj->css() !!}
@endsection

@section('content')
    <?php
        $the_meter = json_decode($the_device);
    ?>

    <div class="wrapper wrapper-content animated fadeInRight">
        <div class="row" id="div_meter_summary" style="margin-bottom:20px;">
            <div class="col-md-6">
                <div class="profile-image">
                    <img src="/img/avatar/client/{{$the_meter->client_avatar}}" class="img-circle circle-border m-b-md" alt="profile">
                </div>
                <div class="profile-info">
                    <div>
                        ...
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.trademark_model') }}:  <strong> {{ $the_meter->trademark." / ". $the_meter->model." (".trans("global.".$the_meter->connection_type).")" }} </strong>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.modem_no') }}:  <a href="/modem_management/detail/{{$the_meter->modem_id}}" target="_blank"><strong> {{ $the_meter->modem_serial }} </strong></a>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.client_distributor') }}:   <a href="/client_management/detail/{{$the_meter->client_id}}" title="{{ trans("devices.go_client_detail") }}" target="_blank"> <strong> {{ $the_meter->client_name }} </strong></a> / <a title="{{ trans("devices.go_distributor_detail") }}" href="/distributor_management/detail/{{$the_meter->distributor_id}}" target="_blank"> <strong> {{ $the_meter->distributor }} </strong></a>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.address') }}:  <strong> {{ $the_meter->location_text }} </strong>
                        </p>
                        <p style="margin: 5px 0 0;">
                            {{ trans('devices.explanation') }}:  <strong>
                                @if( $the_meter->explanation != "" && $the_meter->explanation != null )
                                    {{ $the_meter->explanation }}
                                @else
                ...
                <table class="table small m-b-xs">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>
                                @if ($the_meter->last_connection_at == NULL)
                                    {{ trans('devices.no_first_connection') }}
                                @else
                                    {{ date('d/m/Y H:i:s',strtotime($the_meter->last_connection_at)) }}
                                @endif
                            </td>
                        </tr>
                        <tr>
                            <td>
                                <strong>{{ trans('devices.first_connection') }}</strong>
                            </td>
                            <td>
                                @if ($the_meter->first_connection_at == NULL)
                                    {{ trans('devices.no_first_connection') }}
                                @else
                                    {{ date('d/m/Y H:i:s',strtotime($the_meter->first_connection_at)) }}
                                @endif
                            </td>
                        </tr>
                        ...
                        <tr>
                            ...
                            <td>
                                {{ date('d/m/Y H:i:s',strtotime($the_meter->created_at)) }}
                            </td>
                        </tr>
                        <tr>
                            <td>
                            ...
                            <td>
                                @if ($the_meter->expired_date == NULL)
                                    {{ trans('devices.no_first_connection') }}
                                @else
                                    {{ date('d/m/Y H:i:s',strtotime($the_meter->expired_date)) }}
                                @endif
                            </td>
                        </tr>
                    </tbody>
        ...
        </div> <!-- #div_meter_summary -->

        <div class="row" id="div_meter_tabs">
            <div class="col-lg-12">
                <div class="tabs-container">
                    <ul class="nav nav-tabs" id="meter_detail_tabs">
                        <li class="" tab="#tab-1">
                        ...
                        <div id="tab-1" class="tab-pane">
                            <div class="panel-body tooltip-demo">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.first_data_at") }}: @if ($the_meter->first_data_at == NULL)
                                                    -
                                                @else
                                                    {{ date('d/m/Y H:i:s',strtotime($the_meter->first_data_at)) }}
                                                @endif</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.last_data_at") }}: @if ($the_meter->last_data_at == NULL)
                                                    -
                                                @else
                                                    {{ date('d/m/Y H:i:s',strtotime($the_meter->last_data_at)) }}
                                                @endif</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.multiplier") }}: {{  $the_meter->multiplier }}</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                        <button type="button" class="btn btn-danger btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.meter_time") }}: <?php
                                            $to_time = strtotime($the_meter->last_data_at);
                                            $from_time = strtotime($the_meter->device_time);
                                            echo round(abs($to_time - $from_time) / 60)." ". trans("devices.minute")." ";

                                            if(($to_time-$from_time) >= 0)
                                                echo trans("devices.back");
                                            else if(($to_time-$from_time) < 0){
                                                echo trans("devices.forward");
                                            }
                                            ?></button>
                                        </div>
                                    </div>
                                ...
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.d_period") }}: {{ $the_meter->data_period }} dk </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.invoice_day") }}: {{ $the_meter->invoice_day }}</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{  $the_meter->fee_scale_name }}</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-danger btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{  trans("devices.".$the_meter->fee_scale_type) }} </button>
                                        </div>
                                    </div>
                                </div> <!-- .row -->
                                ...
                                <div class="row" id="div_index_dataTable">
                                    <div class="col-lg-12">
                                        {!!  $IndexDataTableObj->html() !!}
                                    </div>
                        ...
                        <div id="tab-2" class="tab-pane">
                            <div class="panel-body tooltip-demo">
                                <div class="row">
                                    <div class="col-lg-12">
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-primary btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.multiplier") }}: {{  $the_meter->multiplier }}</button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-info btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.current_transformer_ratio") }}: {{$the_meter->current_transformer_ratio }}
                                            </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-warning btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.voltage_transformer_ratio") }}: {{$the_meter->voltage_transformer_ratio }}
                                            </button>
                                        </div>
                                        <div class="col-lg-3 col-md-6">
                                            <button type="button" class="btn btn-danger btn-block m-r-sm" style="cursor: default;margin-bottom: 10px;"> {{ trans("devices.d_period") }}: {{ $the_meter->data_period }} dk</button>
                                        </div>
                                    </div>
                                </div> <!-- .row -->
                        ...
                        <div id="tab-6" class="tab-pane">
                            <div class="panel-body">
                                ...
                                <div class="row">
                                    ...
                                    <div class="col-lg-12" id="div_prepared_graph">
...
@section('page_document_ready')
    ...
    $('#mdgraph_draw_button').click(function(){
        ...
        $.ajax({
            method:"POST",
            url:"/device_management/get_graph_data/{{ $the_meter->device_id }}",
    ...
    rememberTabSelection('#meter_detail_tabs', !localStorage);

    if (document.location.hash && document.location.hash == '#alarms') {
        $("#meter_detail_tabs a[href='#tab-7']").trigger('click');
    }
    else if(document.location.hash){
        $("#meter_detail_tabs a[href='"+document.location.hash+"']").trigger('click');
    }
    ...
    $('#meter_detail_tabs a').on('shown.bs.tab', function(event){
    ...
    active_tab = $('#meter_detail_tabs li.active').attr("tab");
    if( !(active_tab == "" || active_tab == null) )
        load_tab_content(active_tab);
    else
        $("#meter_detail_tabs a:first").trigger('click');
@endsection
