...
class UserManagementController extends Controller
{
        ...
        if(COUNT($result)>0){
            ...
            foreach($result as $one_row){
                ...
                $tmp_array = array(
                    ...
                    "email" => $one_row->email,
                    "org_name" => self::create_org_link($one_row->org_name, $one_row->org_id, $one_row->user_type_id),
    ...
    private function create_org_link($org_name,$org_id,$user_type){

            if($user_type == 3){
                return "<a target='_blank' href='/distributor_management/detail/".$org_id."' title='".trans("user_management.show_distributor_detail")."' >".$org_name."</a>";
            }
            else if($user_type == 4){
                return "<a target='_blank' href='/client_management/detail/".$org_id."' title='".trans("user_management.show_client_detail")."' >".$org_name."</a>";
            }
            else{
                return $org_name;
            }
    ...
    public function create(Request $request){
        ...
        $client_validator = 'bail|digits_between:1,11';
        $distributor_validator = 'bail|digits_between:1,11';
        ...
        if( $request->has('new_user_type') && $request->input("new_user_type") == 4 ){
            $org_id = $request->input("new_user_clients");
            $client_validator = 'bail|required|digits_between:1,11';
            $operations = $default_operations[4];

            //get porg_id (distributor_id) of the client
            $porg_id = DB::table("clients")->where("id",$org_id)->select("distributor_id")->first();
            $porg_id = $porg_id->distributor_id;
        }
        else if( $request->has('new_user_type') && $request->input("new_user_type") == 3 ){
            $org_id = $request->input("new_user_distributors");
            $distributor_validator = 'bail|required|digits_between:1,11';
        ...
        $this->validate($request, [
            ...
            'new_user_clients' => $client_validator,
            'new_user_distributors' => $distributor_validator
        ]);
    ...
    public function getProfile(Request $request){
        $the_user = DB::table('users as U')
            ->select(DB::raw('U.*, UT.type as type, (CASE WHEN U.created_by=0 THEN "'.trans('global.system').'" ELSE UC.name END) as created_by, (CASE WHEN U.user_type=3 THEN D.name WHEN U.user_type=4 THEN C.name ELSE "'.trans('global.main_distributor').'" END) as org_name'))
            ->join('user_type as UT', 'U.user_type', 'UT.id')
            ->leftJoin('distributors as D', 'U.org_id', 'D.id')
            ->leftJoin('clients as C', 'U.org_id', 'C.id')
            ->leftJoin('users as UC', 'U.created_by', 'UC.id')
        ...
        $return_array = array('the_user' => json_encode($the_user));

        if( $the_user->user_type == 3 ){
            $the_organization = DB::table('distributors as D')
                            ->select(
                                'D.*',
                                DB::raw("JSON_UNQUOTE(json_extract(D.location,'$.text')) as location_text"),
                                DB::raw("JSON_UNQUOTE(json_extract(D.location,'$.latitude')) as location_latitude"),
                                DB::raw("JSON_UNQUOTE(json_extract(D.location,'$.longitude')) as location_longitude")
                            )
                            ->where('id',Auth::user()->org_id)
                            ->where('status', '<>',0)
                            ->first();
            $return_array['the_organization'] = json_encode($the_organization);
        }
        else if($the_user->user_type == 4){
            $the_organization = DB::table('clients as C')
                        ->select(
                            'C.*',
                            DB::raw("JSON_UNQUOTE(json_extract(C.location,'$.text')) as location_text"),
                            DB::raw("JSON_UNQUOTE(json_extract(C.location,'$.latitude')) as location_latitude"),
                            DB::raw("JSON_UNQUOTE(json_extract(C.location,'$.longitude')) as location_longitude")
                        )
                        ->where('id',Auth::user()->org_id)
                        ->where('C.status', '<>',0)
                        ->first();
        ...
        //get event logs table from eventlogController
        $eventsTable = new EventlogsController();
        $eventsTable = $eventsTable->prepareEventTableObject($request,"user",Auth::user()->id);
        $return_array["EventDataTableObj"] = $eventsTable;

        return view( 'pages.my_profile', $return_array );
    }
}
